...
<head>
	...
	<style>
		<?php include realpath(dirname(__FILE__) . "/..") . "/assets/styles/global.css" ?>main {
		...
		.studentMessage>button:hover {
			box-shadow: 0 2px 2px 1px rgba(0, 0, 0, 0.8);
		}

		.studentMessage>button:active {
		...
		.administrateur:first-child>.compte,
		.administrateur svg,
		.enseignant:first-child>.compte,
		.enseignant svg {
			cursor: pointer;
		}
		...
		.info {
			position: relative;
			/* les infobulles deviennent référents */
		}

		/* on génère un élément :after lors du survol */
		.info:hover::after {
			content: attr(data-title);
			/* on affiche data-title */
			position: absolute;
			top: -2.9em;
			left: 50%;
...
<body>
	...
	<main>
		...
		<div class="studentMessage">
			<b>Message à afficher aux étudiants sur la page relevés de notes</b>
			<textarea placeholder="Le contenu du message"></textarea>
			<button onclick="sendMessage(this)">Enregistrer</button>
		</div>
	...
	<script>
		...
		async function selectDepartment(departement) {
			...
			let message = await fetchData("getReportPageMessage&dep=" + departement);
			...
			document.querySelector(".studentMessage>textarea").value = message.message.replaceAll('<br>', '\r\n');

			document.querySelector("#departement").classList.remove("highlight");
			document.querySelector(".studentMessage").style.display = "block";
		...
		*/
		...
		/************************************************/
		/* Enregistrement du message pour les étudiants */
		/************************************************/
		async function sendMessage(obj) {
			obj.style.filter = "brightness(0.5)";
			obj.style.pointerEvents = "none";
			let value = obj.previousElementSibling.value.replace(/(?:\r\n|\r|\n)/g, '<br>');
			let reponse = await fetchData(`setReportPageMessage&dep=${selectDep}&text=${value}`);
			if(reponse.reponse == "OK"){
				obj.style.filter = "";
				obj.innerText = "   ✔️   ";
				setTimeout(()=>{
					obj.innerText = "Enregistrer";
					obj.style.pointerEvents = "";
				}, 1000);
			} else {
				message("Une erreur s'est produite, le message n'a peut-être pas été enregistré.")
			}
