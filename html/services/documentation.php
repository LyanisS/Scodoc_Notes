...
function fetchData(query){
	...
	"erreur": "Message d'erreur"
		...
		<div>
			...
			<p>
				Un jeton JWT peut être passé en paramètre POST 'token' pour autoriser l'accès aux services. Ce jeton est à transmettre à chaque requête lors de la communication entre serveurs (et uniquement dans ce cas).<br>
				Voir la section "Authentification aux services".
			</p>
		</div>
	</section>
...
<!----------------------------------------->
	<section>
		...
		<p>
			L'utilisation du CAS est à privilégier pour une interface HTML / CSS (uniquement sur le serveur où l'authentification se fait).<br>
			Le jeton JWT est à utiliser lors d'une communication inter-serveur en le passant en paramètre POST 'token'.<br>
		...
		<p>
			<b>!!! Attention, ce jeton est personnel et permet d'accéder aux services. Il ne doit en aucun cas être public !!!</b>
		</p>
		<div>
			<h3>Query</h3>
...
<!----------------------------------------->	
	<section>
		...
		<p>Pour le moment, cette liste est mise à jour manuellement - si un étudiant n'y est pas listé, veuillez contacter l'administrateur pour une mise à jour et vérification.</p>
		<div>
			<h3>Query</h3>
...
<!----------------------------------------->
	<section>
		...
		<p>Cette requête est une combinaison de l'authentification, la liste étudiant, la liste des semestre et du relevé de notes.</p>
		<p>Elle permet d'avoir toutes les données en une seule requête pour plus de performances. De plus, le nombre de requêtes à Scodoc est limité grâce à une réutilisation en interne des données.</p>
		<div>
			<h3>Query</h3>
